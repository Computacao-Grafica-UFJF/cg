...
import { initRenderer, createGroundPlane, createLightSphere, onWindowResize, radiansToDegrees } from "../libs/util/util.js";
...
function buildInterface() {
    ...
    let controls = new (function () {
        ...
        this.onUpdateShadowFar = function () {
            if (spotLight.shadow.camera.far <= spotLight.shadow.camera.near - 0.1)
                // set far always greater than near
                spotLight.shadow.camera.near = 0.1;
            updateLight();
        };
        this.onUpdateShadowNear = function () {
            if (spotLight.shadow.camera.near >= spotLight.shadow.camera.far)
                // set near always smaller than far
    ...
    spotFolder.open();
    spotFolder.add(spotHelper, "visible", true).name("Helper");
    spotFolder.add(spotLight, "intensity", 0, 5);
    spotFolder.add(spotLight, "penumbra", 0, 1);
    spotFolder.add(spotLight, "distance", 0, 40, 0.5).onChange(function () {
        updateLight();
    });
    spotFolder
        .add(controls, "angle", 20, 60)
        .name("Angle")
        .onChange(function () {
            controls.onUpdateLightAngle();
        });
    ...
    shadowFolder.open();
    shadowFolder.add(shadowHelper, "visible", true).name("Helper");
    // Controls if the shadow needs to be updated
    shadowFolder
        .add(renderer.shadowMap, "autoUpdate", true)
        .onChange(function () {
            controls.onUpdateShadowNear();
        })
        .name("Auto Update");
    shadowFolder.add(controls, "shadowMapSize", 16, 1024, 16).onChange(function () {
        controls.onUpdateShadowMap();
    });
    shadowFolder
        .add(spotLight.shadow.camera, "near", 0.1, 30, 0.1)
        .onChange(function () {
            controls.onUpdateShadowNear();
        })
        .listen(); // Change GUI when the value changes outside
    shadowFolder
        .add(spotLight.shadow.camera, "far", 0.1, 30, 0.1)
        .onChange(function () {
            controls.onUpdateShadowFar();
        })
        .listen();
}
